...
class Map(object):
    ...
    def cartesian_to_frenet(self, x, y):
        ...
        for _, road_id in nearest_road_id:
            ...
            if t_current > 10 or t_current < -10 or s == 0:
                continue
            else:
...
class Text(object):
    ...
    def set_text(self, text, color=COLOR_WHITE):
        ...
        self.surface = pygame.Surface(self.dim, pygame.SRCALPHA)
        self.surface.fill((192,192,192,127))
...
class World(object):
    ...
    def update_info(self):
        ...
        for vehicle_id in self.vehicle_record[self.clock_tick]:
            # why (1, 9, 10) is not in vehicle_running?
            if (vehicle_id == (1,9,10)):
                continue
            sum_velocity += self.vehicle_running[vehicle_id].vel
            self.veh_count +=1
        if self.veh_count >= 1:
    ...
    def show_road_information(self):
        ...
        s = " Map: {map_name}\n Time:{time}\n " \
            ...
            "Vehicle count: {veh_count}\n Average speed: {av_speed}\n ".format(map_name=self.map_name, time=self.clock_tick,
                                                          road_count=len(self.map.roads),junc_count=len(self.map.junctions),
                                                          veh_count=self.veh_count, av_speed=self.average_speed)
        self.text.surface = pygame.Surface(self.text.dim, pygame.SRCALPHA)
        self.text.surface.fill((192, 192, 192, 127))
        if road_id >= 0:
            s += "Road id: {road}.\n Predecessor: {pre}\n Successor: {suc}\n " \
                 "Length: {l}".format(road=str(road_id),
        ...
        words = [word.split(' ') for word in s.splitlines()]

        space = self.text.font.size(' ')[0]  # The width of a space.
        max_width, max_height = self.text.surface.get_size()
        # x, y = self.text.pos
        x, y = 5, 5
        for line in words:
            for word in line:
                word_surface = self.text.font.render(word, 0, COLOR_WHITE)
                word_width, word_height = word_surface.get_size()
                if x + word_width >= max_width:
                    x = 5  # Reset the x.
                    y += word_height  # Start on new row.
                self.text.surface.blit(word_surface, (x, y))
                x += word_width + space
            x = 5  # Reset the x.
            y += word_height  # Start on new row.
    ...
    def visulizer(self, input_control):
        ...
        self.text = Text(pygame.font.Font(pygame.font.get_default_font(), 16),
                         (int(0.3*screen_width), int(0.5*screen_width)), (int(0.7*screen_width), int(0.05*screen_width)))
        self.str_junction = ''
        self.info_junction_id = -1
        # world_image start
        ...
        suspend_button = Button(0, 0, int(0.1 * screen_width), int(0.05 * screen_width), 'Suspend', suspend)
        def restart():
            self.clock_tick = 0
        restart_button = Button(int(0.1 * screen_width), 0,
        ...
        restore_button = Button(int(0.2 * screen_width), 0,
        ...
        while (True):
            ...
            if right_click_junction_id >= 0 and self.focus_on:
                ...
                input_control.wheel_offset = 1 / (self.map._world_width / 120)
            ...
            pygame.draw.rect(display, COLOR_INACTIVE, pygame.Rect(0, 0, screen_width, int(0.05 * screen_width)))
...
def simulation():
    run_time = 160  # running time for simulation
