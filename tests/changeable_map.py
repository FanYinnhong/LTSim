...
class World(object):
    ...
    def visulizer(self, input_control):
        ...
        self.mouse_x = None
        self.mouse_y = None
        # self.prev_scaled_size = int(world_image._width_in_pixels)
        # self.world_size = int(world_image._width_in_pixels)
        # self.x_window = 0
        # self.y_window = 0
        # self.x_mid_window = int(world_image._width_in_pixels / 2)
        # self.y_mid_window = int(world_image._width_in_pixels / 2)
        # handle_surface = world_image.big_map_surface.copy()
        # handle_surface.set_clip(self.compute_scale(input_control))
        ...
        for t in range(self.run_time):
            ...
            #     input_control._parse_mouse(x, y)
            display.fill(COLOR_ALUMINIUM_4)
            world_image.surface.blit(world_image.big_map_surface, (0, 0))
            world_image.draw_vehicle(t)


            # image = world_image.surface.subsurface(handle_surface.get_clip())
            # image = world_image.surface.subsurface(self.compute_scale(input_control))
            # x_display = int(screen_width / input_control.wheel_offset)
            # y_display = int(screen_width / input_control.wheel_offset)
            # self.compute_scale(input_control)
            # display.blit(pygame.transform.scale(world_image.surface,
            #                                     (self.scaled_size, self.scaled_size)), (-self.x_window, -self.y_window))
    ...
    # def compute_scale(self, input_control):
    #     scale_factor = input_control.wheel_offset
    #     # when scale factor is smaller, the world seems bigger
    #     self.scaled_size = int(self.world_size * scale_factor)
    #     if self.scaled_size != self.prev_scaled_size:
    #     # if the surface is zoomed
    #         # left top conner collision
    #         if (self.x_mid_window - self.scaled_size / 2 < 0 or self.y_mid_window - self.scaled_size / 2):
    #             self.scaled_size = min(self.x_mid_window, self.y_mid_window) - 3
    #
    #         if (self.x_mid_window + self.scaled_size / 2 > self.world_size or self.y_mid_window + self.scaled_size / 2 > self.world_size):
    #             self.scaled_size = min(self.world_size - self.x_mid_window, self.world_size - self.y_mid_window) - 3
    #
    #         self.x_window = int(self.x_mid_window - self.scaled_size / 2)
    #         self.y_window = int(self.y_mid_window - self.scaled_size / 2)
    #         print(self.x_window)
    #         # self.x_mid_window = self.x_window + int(self.scaled_size / 2)
    #         # self.y_mid_window = self.y_window + int(self.scaled_size / 2)
    #
    #     # if the surface is dragged
    #     # if (input_control.mouse_pos_rel[0] != 0 or input_control.mouse_pos_rel[1] != 0):
    #     #     self.x_window -= input_control.mouse_pos_rel[0]/scale_factor
    #     #     self.x_mid_window -= input_control.mouse_pos_rel[0] / scale_factor
    #     #     self.y_window -= input_control.mouse_pos_rel[1] / scale_factor
    #     #     self.y_mid_window -= input_control.mouse_pos_rel[1] / scale_factor
    #     self.prev_scaled_size = self.scaled_size
    #     return pygame.Rect(self.x_window, self.y_window, self.scaled_size, self.scaled_size)
