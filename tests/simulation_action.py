import threading
import math, os, sys, pdb, time, random
        ...
        for t in range(self.run_time):
            for vehicle_info in flow:
                ...
                if temp_id not in self.vehicle_demand and arrival_time > t:
                    scf = 0
                    if from_road_id in [3,7]:   
                        lane_number = random.randint(1, 2) * -1
                    else:                      
    ...
    def action(self):
        ...
        for t in range(self.run_time):
            ...
            for vehicle_id in self.vehicle_running:
                ...
                if len(vehicle.plan_tra) <= vehicle.plan_period - vehicle.plan_gap or vehicle.plan_road_id != vehicle.road_id or tra_check(vehicle, vehicle.plan_tra, vehicle.sur_traffic, 0.6):
