...
class Map(object):
    ...
    def get_junction_coor(self):
        for junction in self.junctions:
            ...
            for connection in junction.connections:
                road = self.roads[self._road_id_to_index[connection.connectingRoad]]
    ...
    def collision_check(self, x, y):
        ...
        nearest_start_idx = self.nearest_start_points(x, y)

        for _, start_idx in nearest_start_idx:
            # print(start_idx)
            geometries = self.roads[start_idx].planView.geometries
            cnt_driving_left, sidewalk_left, shoulder_left = classify_lane_type(self.roads[start_idx].lanes.laneSections[0].leftLanes)
            cnt_driving_right, sidewalk_right, shoulder_right = classify_lane_type(
                self.roads[start_idx].lanes.laneSections[0].rightLanes)
            ...
            for geometry in geometries:
                ...
                if is_in:
                    return self.roads[start_idx].id
        return -1
    ...
    def nearest_start_points(self, x, y):
        ...
        for idx, road in enumerate(self.roads):
            ...
            nearest_start_idx.append([temp, idx])
        heapq.heapify(nearest_start_idx)

        return nearest_start_idx
                    ...
                    if (scf < 0):
                        ...
                        # if next is a road
                            ...
                            if road.link.predecessor.contactPoint == "start":
                                ...
                                scf = map.roads[
                                          map._road_id_to_index[road.link.predecessor.elementId]].length - scf_temp
                                direction = 1
                                scf_end = 0
                                # lane = 1
    ...
    def visulizer(self, input_control):
        ...
        while (True):
                ...
                if event.type == pygame.QUIT:
                    ...
                    if event.button == 1:
                        ...
                        if double_click_timer == 0:
                            ...
                            # print(d_road_id)
                            if d_road_id in self.map._road_id_to_junction_index:
                                double_junction_index = self.map._road_id_to_junction_index[d_road_id][0]
                            double_click_timer = 0
            ...
            if double_junction_index >= 0:
                ...
                x_temp, y_temp = self.map.junctions[double_junction_index].pos_left_top
