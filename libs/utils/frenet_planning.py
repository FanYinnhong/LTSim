...
def Bezier_fitting(status_0, status_t, period):
    ...
    def Bezier(p,v_arr):
        ...
        res = []
        k = 0
        for v_i in range(len(v_arr)):
            v_rate = sum(v_arr[0:v_i+1])/max(sum(v_arr), 0.01)
            x, y = 0, 0
            for i in range(n+1):
                x += B(n,i,v_rate) * p[i][0] 
                y += B(n,i,v_rate) * p[i][1] 
            if len(res)>0 :
                k = np.arctan((y - y_p)/(x - x_p+0.0001))
                if abs(k) < 0.001:
                    k = 0
            x, y, k = round(x, 3), round(y, 3), round(k, 3)
            res.append([x,y,v_arr[v_i],k])
            x_p, y_p = x, y
        return res
    px_0,py_0,v0,theta_0 = status_0
    px_t,py_t,vt,vc_t = status_t
    steps =  round(vc_t * 10)
    v_arr = []
    for i in range(1, steps + 1):
        v = v0 * (1 - i/steps) + vt * i/steps
        v_arr.append(v)
    for i in range(steps + 1, round(period * 10) + 1):
        v_arr.append(vt)
    Bp = [(px_0,py_0), (1/2*(px_0 + px_t), py_0), (1/2*(px_0 + px_t), py_t), (px_t, py_t)]
    B_Tra = Bezier(Bp, v_arr)
    return B_Tra
...
def get_obj_point(vehicle, scf, tcf, vel, PLAN_TIME, scf_end, sampling_times = 1):
    ...
    def sampling_process(sample, permutation):
        if permutation == []:
            ...
            return ret
    # acc_sample = [vehicle.ACC_MAX * (random.random()*0.5 + 0.5), 0, -vehicle.DCC_MAX]
    acc_sample = list(np.arange(vehicle.ACC_MAX, -vehicle.DCC_MAX-1, -0.5))
    acc_permutation = []
    tcf_list = [2, 6]
    tcf_permutation = []
    ...
    for action in acc_permutation:
        ...
        for acc in action:
            ...
            if acc != 0: 
                ...
                t = PLAN_TIME
            st = s0 + (v0 + vt)/2 * t + vt * (PLAN_TIME - t)  
            if (vt + 0)/2 * min(PLAN_TIME, vt/vehicle.DCC_MAX)  > (scf_end - 5 - st):
                break
            if s0 < scf_end - 3 and st > scf_end - 5:
                st = max(scf_end - 5, s0)
                vt = 0
            sub_obj_points.append([st, tcf, vt, t])
            s0, v0 = st, vt      
    ...
    for acc in last_action:
        ...
        if s0 < scf_end - 3 and st > scf_end - 5:
            st = max(scf_end - 5, s0)
            vt = 0
        last_choice.append([st, tcf_min, vt, t])
        s0, v0 = st, vt   
...
def is_collide_point(ego, pos_1, other, pos_2, type = 0):  
        ...
        for p1 in ps_1:
            for p2 in ps_2:
                if np.hypot(p1[0]-p2[0], p1[1]-p2[1]) < 2:
                    return True
        return False
    if round(np.hypot(pos_1[0]-pos_2[0], pos_1[1]-pos_2[1])) <= 8:
        attitude1 = [pos_1[0], pos_1[1], pos_1[3]]
        attitude2 = [pos_2[0], pos_2[1], pos_2[3]]
        if type:
            return True
        elif point2point(get_shape_pos(ego, attitude1), get_shape_pos(other, attitude2)):
            return True
    return False    
...
def is_cross_tra(ego, tra_1, other, tra_2, threshold):
    for i in range(len(tra_1)):
        ...
        dis_acc = s_1 - s_0
        if abs(t_0 - t_1) < 2: # in the same lane
            if tra_1[0][0] < tra_2[0][0]: # ego car is the following car
                risk = (2 * rt * v_0_m + v_0_m ** 2 / (2 * ego.DCC_MAX) - v_1_m**2/(2 * other.DCC_MAX) + ego.vehicle_l + 4) / (dis_acc + 0.0001)
                if risk > threshold or is_collide_point(ego, p0, other, p1, 1):
                    return True
        else: # in the different lanes 
            if abs(tra_1[0][1] - tra_1[-1][1]) > 2: # lane changing constraints
...
class Planner():
    def __init__(self, ego):
        self.ego = ego
    def tra_check(self, cur_tra, threshold = 0.8):    
        for sur_car_id in self.ego.sur_traffic:
            if sur_car_id in self.ego.world.vehicle_running:
                sur_car = self.ego.world.vehicle_running[sur_car_id]
                if is_cross_tra(self.ego, cur_tra, sur_car, self.ego.sur_traffic[sur_car_id], threshold):
                    return True
        return False
    def planning(self, motion_status, period):
        ...
        for obj_point in obj_points:
            ...
            if obj_point == last_choice_p:
                best_tra = cur_tra
            if not self.tra_check(cur_tra) and len(cur_tra) > 0:            # check trajectories
                best_tra = cur_tra
                break     
        return best_tra